...
import { reservationBanquetApi, banquetApi, paymentApi } from "../../lib/api";
...
export default function BookingPaymentBanquet() {
  ...
  // ชั่วโมงที่ใช้งาน
  const hours = useMemo(
    () => diffHours(data?.event_date, data?.start_time, data?.end_time),
    [data]
  );
  ...
  async function handleUpload(e) {
    ...
    try {
      ...
      nav(`/bookings/success-banquet?code=${encodeURIComponent(code)}`, { replace: true });
    } catch (e2) {
      ...
      if (msg.includes("DUPLICATE_SLIP") || msg.includes("สลิปนี้เคยถูกใช้")) {
        icon = "warning"; title = "สลิปนี้ถูกใช้ไปแล้ว";
        text = "กรุณาอัปโหลดสลิปใหม่ที่ยังไม่เคยตรวจสอบมาก่อน"; setErr("");
      } else if (msg.includes("INVALID_FILE_TYPE")) {
        icon = "error"; title = "ไฟล์ไม่รองรับ";
          ...
          <div className="bpGrid">
            ...
            <aside className="bpCard">
              ...
              <dl className="bpList">
                ...
                <div>
                  <dt>วัน–เวลา</dt>
                  <dd>{fmtDate(data?.event_date)} ({fmtTime(data?.start_time)}–{fmtTime(data?.end_time)}) • {hours} ชม.</dd>
                </div>
            ...
            <section className="bpCard">
              ...
              {(data?.last_payment_status === "unpaid" || data?.last_payment_status === "rejected") && (
                <form className="bpPayForm" onSubmit={handleUpload}>
                  {/* ===== ลบส่วน 'ยอดที่ต้องชำระ (บาท)' ฝั่งขวาเรียบร้อย ===== */}
